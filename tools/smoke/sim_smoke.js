...
export async function run(t) {
  ...
  t.ok(gjkIntersect(sa, sb).hit, 'overlapping spheres intersect');
  t.ok(!gjkIntersect(sa, sphereSupport([3,0,0], 1)).hit, 'separated spheres disjoint');
  const bx = boxSupport([0,0,0], [1,1,1]);
  t.ok(gjkIntersect(bx, sphereSupport([0.5,0,0], 1)).hit, 'box overlaps inner sphere');
  t.ok(!gjkIntersect(bx, sphereSupport([5,0,0], 1)).hit, 'box far from sphere');
