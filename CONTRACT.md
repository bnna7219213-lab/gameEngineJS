...
| DOM 隔离 | 只有 `src/editor/**` 允许触碰 `document`/`window`/`canvas`。引擎层禁止引用 DOM（除 RHI 后端的 canvas 参数）。`src/engine/vdom/dom_host.js` 仅在**被调用时**才接触 `document`（模块级不引用、import 零副作用），由调用方注入 documentRef，不违反本条。 |
