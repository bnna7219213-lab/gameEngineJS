...
> **2026-09-08 进度**：M-A0 + M-A 数据层已落地 ✅（58 smoke / 1800 断言全绿，零回退）：
> - M-A0：H1 id 分裂修复（GameObject3D 接受 id 参数 + deserialize 恢复原 id）、
>   H2 children 腐化修复（remove 同步清理父 children + removeRecursive）、
>   H7 core/hash.js（fnv1a + stableStringify + sceneHash/meshHash/commandHash）。
> - M-A：hedit.js（half-edge 内核：fromRenderMesh weld 24→8 / toRenderMesh 三角化+Newell 法线 /
>   splitEdge/deleteFace/mergeVerts/validate 6 项不变量）、modifier.js（MIRROR/ARRAY/SOLIDIFY/
>   SUBDIVIDE/WELD/DECIMATE 真实现 + TRIANGULATE/BEVEL 占位）、material.js + world.js（数据块+
>   MaterialLibrary+序列化往返）、commands.js（Command/SetValue/Snapshot + CommandBus：
>   undoGroup try/finally 闭合 + dirtySet/consumeDirty）、mode.js（OBJECT/EDIT_VERT/EDGE/FACE
>   + 快捷键按模式路由）、registry 扩展（+7 字段类型 + 13 属性选项卡 + tabOfComponent）、
>   history.js 命令级双轨（CommandHistory 挂 CommandBus，既有快照接口不变）。
> - 新增 smoke（8 个 / 204 断言）：core_hash / plat_scene_id / plat_modeling_hedit /
>   plat_modeling_modifier / plat_modeling_undo / plat_material / editor_mode / editor_tabs。
> - 待续：M-A 的 GUI 接入（视口 dirty 重建 + 面板渲染）属 M-B 范围。
...
> **2026-09-08 进度**：M-B 数据层+核心交互已落地 ✅（63 smoke / 1903 断言全绿，零回退）：
> - H5 修复：gizmo.js 删除 _move 死代码，抽纯函数 axisWorldDelta；gizmo 补全 rotate/scale
>   模式（hitCircle/angleDelta/axisScaleFactor/hitSegment 纯函数 Node 可测）。
> - H8 修复：viewport3d.js 资源缓存化（_meshCache 按 mesh.id+指纹复用 vb/ib，pbr shader/pipeline
>   单例，等长位置编辑走 writeBuffer 原位更新），stats 统计，输出逐位不变。
> - P0 算子命令化（ops.js）：ExtrudeRegion/MergeVerts/Delete/Subdivide 命令类 + createMeshOps
>   便捷 API；GUI 与脚本共用命令总线，do/undo meshHash 往返一致。
> - 编辑模式拾取（pick.js）：MeshPicker 顶点/边/面选择（Möller–Trumbore 射线-三角形 +
>   屏幕空间距离），纯函数 Node 可测。
> - 编辑会话（edit_session.js）：EditSession 装配模式状态机+选择+命令总线+HMesh，
>   进/出编辑模式互转、拾取选择、便捷算子（extrude/deleteSelected/mergeSelected/subdivide）。
> - 视口 dirty 重建（viewport.js）：syncScene 接入 CommandBus.consumeDirty，增量重建受影响对象。
> - 新增 smoke（5 个 / 103 断言）：render_viewport_cache(17) / editor_gizmo(27) /
>   plat_modeling_ops(19) / plat_modeling_pick(20) / editor_modeling_session(20)。
...
> **2026-09-08 进度**：M-C 已落地 ✅（64 smoke / 1925 断言全绿，零回退）：
> - api.js：createModelingApi 返回 data/ctx/ops/undoGroup；data 含 meshes/materials/worlds
>   数据块（materials 接 MaterialLibrary，worlds 接 World.default）；ctx 含 activeObject/
>   selectedObjects/mode/selectMode（只读 getter）；ops 含 object.modeSet/mesh.*/transform.*。
> - runScript 注入：把 api 展开为局部变量（data/ctx/ops/undoGroup）后执行，脚本内
>   `ops.mesh.selectAll("SELECT"); ops.mesh.extrudeRegion({ offset: 0.2 })` 与 GUI 按钮
>   走同一命令总线，do/undo meshHash 一致。
> - undoGroup：脚本内 `undoGroup(() => { for ... })` 合并为单步撤销。
> - 新增 smoke：editor_modeling_api(22 断言)。
...
> **2026-09-08 进度**：M-D 已落地 ✅（66 smoke / 1959 断言全绿，零回退）：
> - constraint.js（sim 层，零 DOM）：COPY_LOCATION/ROTATION/SCALE/TRANSFORMS（influence 插值）、
>   TRACK_TO（yaw/pitch 朝向）、LIMIT_LOCATION/ROTATION/SCALE（逐轴钳制）；solveConstraints
>   场景级求解；未知类型/缺 target 抛可读错误（红线 A）。
> - modifier_cmd.js：obj.modifiers 栈挂载（modifiersOf/evalModifiedMesh 惰性求值）+
>   AddModifier/RemoveModifier/MoveModifier（顺序敏感）/SetModifierParam（属性回滚）/
>   ApplyModifier（求值烘回基础 HMesh + 栈移除，undo 恢复快照+栈）命令类。
> - 新增 smoke：sim_constraint(15) / plat_modeling_modifier_cmd(19)。
> - 已知坑记录：cube 关于 X=0 对称，MIRROR X 后镜像顶点与原顶点重合被 weld（测试须先移离对称面）。
> - 待续：修改器/约束选项卡 UI 渲染（registry tabs 已有框架）与 Play 模式约束每帧求解接线。
...
> **2026-09-08 进度**：M-E 已落地 ✅（70 smoke / 2035 断言全绿，零回退）：
> - 物理选项卡（M9）：physics_binding.js —— rigidbody/collider3d 组件 -> PhysicsWorld 绑定
>   （shapeFromCollider/bodyFromObject/PhysicsSession：sync/step/回写 transform pos+欧拉旋转）。
>   静态刚体、质量/摩擦/恢复映射、重力下落、碰撞支撑、remove 清理均断言。
> - 粒子扩展（M8）：particles.js 加发射器形状（sphere/cone/box/edge/point）+ 力场
>   （gravity/wind/vortex/attract，radius 线性衰减）+ 生命周期曲线（sizeCurve/colorCurve 关键帧插值）
>   + 朝向对齐（alignToVelocity）；point 形状 Rng 序列与旧版逐字节一致（确定性兼容）。
> - 材质 normalMap（M10）：pbr.js 加 computeTangents（Gram-Schmidt 正交化+手性）+ perturbNormal
>   （TBN 变换）+ pbrShade normalMap 分支 + GLSL PBR_VS/FS 对等（aTangent location=3 可选属性）。
>   无 normalMap/tangent 时渲染逐位不变；render_pbr_parity 等全绿。
> - 世界下发（M7）：viewport.js applyWorldToViewport（ambient/fog/ibl/sunDir->方向光），
>   World.default() 向后兼容。
> - registry 注册 rigidbody/particle/modifier/constraint 组件（物理/粒子选项卡参数可见）。
> - 新增 smoke（4 个 / 76 断言）：editor_physics(17) / sim_particles_ext(21) /
>   render_normalmap(29) / plat_world_viewport(9)。
> - 待续：World/材质选项卡 UI 渲染（手动验收）与 Play 模式物理/约束每帧求解接线（v2 Q1 范畴）。
