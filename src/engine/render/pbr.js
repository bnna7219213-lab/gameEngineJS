...
// ----------------------------------------------------------- 切线计算（M10）
// 标准逐三角形累加 T/B，逐顶点 Gram-Schmidt 正交化 + 手性 w = sign(dot(cross(N,T),B))。
// positions/normals/uvs 为交错 Float32Array（3/3/2 分量），indices 可空（非索引网格）。
// 返回 Float32Array，每顶点 4 分量 [tx,ty,tz,w]。uv 退化三角形跳过（该顶点回退到任意正交切线）。
export function computeTangents(positions, normals, uvs, indices) {
  const n = positions.length / 3;
  const T = new Float32Array(n * 3), B = new Float32Array(n * 3);
  const idx = indices || Array.from({ length: n }, (_, i) => i);
  for (let t = 0; t + 2 < idx.length; t += 3) {
    ...
    const u1 = uvs[i1 * 2] - uvs[i0 * 2], v1 = uvs[i1 * 2 + 1] - uvs[i0 * 2 + 1];
    const u2 = uvs[i2 * 2] - uvs[i0 * 2], v2 = uvs[i2 * 2 + 1] - uvs[i0 * 2 + 1];
    const det = u1 * v2 - u2 * v1;
    if (Math.abs(det) < 1e-12) continue; // uv 退化
    const r = 1 / det;
    const tdir = [(e1[0] * v2 - e2[0] * v1) * r, (e1[1] * v2 - e2[1] * v1) * r, (e1[2] * v2 - e2[2] * v1) * r];
    const bdir = [(e2[0] * u1 - e1[0] * u2) * r, (e2[1] * u1 - e1[1] * u2) * r, (e2[2] * u1 - e1[2] * u2) * r];
    for (const i of [i0, i1, i2]) {
      T[i * 3] += tdir[0]; T[i * 3 + 1] += tdir[1]; T[i * 3 + 2] += tdir[2];
      B[i * 3] += bdir[0]; B[i * 3 + 1] += bdir[1]; B[i * 3 + 2] += bdir[2];
    }
  }
  const out = new Float32Array(n * 4);
  for (let i = 0; i < n; i++) {
    const N = norm3([normals[i * 3], normals[i * 3 + 1], normals[i * 3 + 2]]);
    let t = [T[i * 3], T[i * 3 + 1], T[i * 3 + 2]];
    let b = [B[i * 3], B[i * 3 + 1], B[i * 3 + 2]];
    if (Math.hypot(t[0], t[1], t[2]) < 1e-9) {
      // 无有效 uv：任取与 N 正交的轴
      const ax = Math.abs(N[0]) < 0.9 ? [1, 0, 0] : [0, 1, 0];
      t = cross3(N, ax); b = cross3(N, t);
    }
    // Gram-Schmidt：T' = normalize(T - N*dot(N,T))
    const dnt = dot3(N, t);
    const tg = norm3([t[0] - N[0] * dnt, t[1] - N[1] * dnt, t[2] - N[2] * dnt]);
    const w = dot3(cross3(N, tg), b) < 0 ? -1 : 1;
    out[i * 4] = tg[0]; out[i * 4 + 1] = tg[1]; out[i * 4 + 2] = tg[2]; out[i * 4 + 3] = w;
  }
  return out;
}

// 法线贴图解码 + TBN 扰动（CPU 参考，与 GLSL uUseNormalMap 路径同式）。
// normalMap: [r,g,b]（0..1 或 0..255 均可，>1 视为 0..255）或纹理 {w,h,data}（配合 uv）。
// tangent: [x,y,z] 或 [x,y,z,w]（w 为手性）。bitangent 可显式给出，否则 cross(N,T)*w。
export function perturbNormal(N, normalMap, tangent, bitangent, uv) {
  let texel = normalMap;
  if (!Array.isArray(normalMap)) texel = sampleTexture(normalMap, uv || [0, 0]); // 0..255
  const s = (texel[0] > 1 || texel[1] > 1 || texel[2] > 1) ? 255 : 1;
  const tn = [texel[0] / s * 2 - 1, texel[1] / s * 2 - 1, texel[2] / s * 2 - 1];
  const T = norm3([tangent[0], tangent[1], tangent[2]]);
  const w = tangent.length > 3 ? tangent[3] : 1;
  const B = bitangent ? norm3(bitangent) : arrMul(cross3(N, T), w);
  return norm3([
...
export function pbrShade(opts) {
  let Nn = norm3(opts.N); const Vv = norm3(opts.V);
  // 可选：法线贴图扰动（M10）。无 normalMap/tangent 时与现状逐位一致。
  if (opts.normalMap && opts.tangent) Nn = perturbNormal(Nn, opts.normalMap, opts.tangent, opts.bitangent, opts.uv);
...
layout(location=3) in vec4 aTangent; // 可选（M10 法线贴图）；不提供时取属性默认值，FS 不启用
uniform mat4 uModel; uniform mat4 uViewProj; uniform mat4 uView;
out vec3 vWorld; out vec3 vNormal; out vec2 vUV; out float vViewZ; out vec4 vTangent;
void main(){
...
in vec3 vWorld; in vec3 vNormal; in vec2 vUV; in float vViewZ; in vec4 vTangent;
out vec4 o;
...
uniform float uUseNormalMap; uniform sampler2D uNormalMap; // M10 法线贴图（可选）
...
void main(){
  ...
  // M10：可选法线贴图扰动（与 js perturbNormal 同式）
  ...
  return {
    ...
    js: {
      vs: (a, uni) => {
        ...
        let tg = null;
        if (a.tangent) { const tv = uni.model.transformDir(Vec3.of(a.tangent[0], a.tangent[1], a.tangent[2])).normalize(); tg = [tv.x, tv.y, tv.z, a.tangent[3] == null ? 1 : a.tangent[3]]; }
        return { pos: clip, vary: { wp: [w.x, w.y, w.z], wn: [n.x, n.y, n.z], uv: a.uv || [0, 0], tangent: tg } };
      },
      fs: (v, uni) => {
        const N = norm3(v.wn);
        ...
        const c = pbrShade({ N, V, P: v.wp, albedo, metallic: uni.metallic || 0, roughness: uni.roughness == null ? 0.8 : uni.roughness, emissive: uni.emissive || [0, 0, 0], lights: uni.lights || [], ambient: uni.ambient || [0.04, 0.05, 0.07], fog: uni.fog || null, dist, shadow, normalMap: uni.normalMap || null, tangent: v.tangent, uv: v.uv });
