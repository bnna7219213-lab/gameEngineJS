...
export class Viewport3D {
  constructor(device, { width = 320, height = 240 } = {}) {
    ...
    // H8 渲染资源缓存：脏才重建 vb/ib；shader/pipeline 单例（_shader/_pbr 与布局固定）。
    this._meshCache = new Map(); // mesh.id -> { key, vb, ib, vertCount, indexCount, lit }
    this._litShaderCache = null;   // { sh, pipe } pbr 单例
    this._unlitShaderCache = null; // { sh, pipe } unlit 单例（shader 不依赖 albedo，颜色走 uniforms）
    this.stats = { bufferCreates: 0, pipelineCreates: 0, cacheHits: 0 };
    this._ensureDevice();
  }
  async _ensureDevice() {
  ...
  removeMesh(id) { this.meshes.delete(id); this._meshCache.delete(id); }
...
void main(){
  ...
  // 内容指纹：长度 + 首末顶点抽样 + albedo；变则脏，脏才重建 GPU 缓冲。
  _meshKey(mesh, lit) {
    const p = mesh.positions;
    return `${lit ? 1 : 0}|${p.length}|${mesh.indices.length}|${p[0]},${p[p.length - 1]}|${mesh.albedo}`;
  }
  // 取 mesh 的 vb/ib：缓存命中复用；顶点/索引数未变走 writeBuffer 原位更新；否则重建。
  _meshResources(mesh, lit) {
    const key = this._meshKey(mesh, lit);
    let e = this._meshCache.get(mesh.id);
    if (e && e.key === key) { this.stats.cacheHits++; return e; }
    const vdata = lit ? packLit(mesh.positions, mesh.normals) : packInterleaved(mesh.positions, mesh.normals);
    const vertCount = mesh.positions.length / 3;
    const indexCount = mesh.indices.length;
    let vb, ib;
    if (e && e.lit === lit && e.vertCount === vertCount) { this.device.writeBuffer(e.vb, vdata); vb = e.vb; }
    else { vb = this.device.createBuffer({ data: vdata }); this.stats.bufferCreates++; }
    if (e && e.lit === lit && e.indexCount === indexCount) { this.device.writeBuffer(e.ib, mesh.indices); ib = e.ib; }
    else { ib = this.device.createBuffer({ data: mesh.indices }); this.stats.bufferCreates++; }
    e = { key, vb, ib, vertCount, indexCount, lit };
    this._meshCache.set(mesh.id, e);
    return e;
  }
  // shader+pipeline 单例：unlit 与 lit 各一份（着色代码与布局固定，颜色/光照走 uniforms，逐位不变）。
  _unlitPipeline() {
    if (!this._unlitShaderCache) {
      const sh = this.device.createShader(this._shader());
      this._unlitShaderCache = { sh, pipe: this.device.createPipeline({ shader: sh, vertexLayout: vlayout }) };
      this.stats.pipelineCreates++;
    }
    return this._unlitShaderCache;
  }
  _litPipeline() {
    if (!this._litShaderCache) {
      const sh = this.device.createShader(this._pbr);
      this._litShaderCache = { sh, pipe: this.device.createPipeline({ shader: sh, vertexLayout: this._litLayout, depth: true, cull: 'none' }) };
      this.stats.pipelineCreates++;
    }
    return this._litShaderCache;
  }
  ...
  _renderUnlit() {
    ...
    const pipe = this._unlitPipeline().pipe;
    for (const mesh of this.meshes.values()) {
      const res = this._meshResources(mesh, false);
      ...
      this.device.setVertexBuffer(res.vb);
      this.device.setIndexBuffer(res.ib);
  ...
  _renderLit() {
    ...
    const pipe = this._litPipeline().pipe;
    for (const mesh of this.meshes.values()) {
      const res = this._meshResources(mesh, true);
      const world = mesh.transform ? Mat4.compose(Vec3.of(...(mesh.transform.position || [0, 0, 0])), Vec3.of(...(mesh.transform.rotation || [0, 0, 0])), Vec3.of(...(mesh.transform.scale || [1, 1, 1]))) : Mat4.identity();
      dev.setPipeline(pipe);
      dev.setVertexBuffer(res.vb);
      dev.setIndexBuffer(res.ib);
      ...
      // Q3（G6）：GL 路径 fog uniforms 下发（此前只有 Software 路径经 uni.fog 生效）。
      // 展开规则与 pbrShade 的 fog 语义一致：type 0=无 1=linear 2=exp。
      const fog = this.fog;
      if (fog) {
        uni.uFogType = fog.type === 'exp' ? 2 : 1;
        uni.uFogColor = fog.color || [1, 1, 1];
        uni.uFogNear = fog.near != null ? fog.near : 0;
        uni.uFogFar = fog.far != null ? fog.far : 100;
        uni.uFogDensity = fog.density != null ? fog.density : 0;
      } else {
        uni.uFogType = 0;
      }
