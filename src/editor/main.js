...
import { defaultOutputSettings } from './registry.js';
...
const commands = [
  ...
  { id: 'export-obj', title: '导出场景 OBJ（+MTL）', run: () => exportSceneFile('obj') },
  { id: 'export-gltf', title: '导出场景 glTF（.gltf）', run: () => exportSceneFile('gltf') },
  { id: 'export-glb', title: '导出场景 glTF（.glb）', run: () => exportSceneFile('glb') },
...
// v3 M-F：场景导出（OBJ/glTF/GLB）。坐标/单位与旋转单位取自「输出」选项卡设置
// （project.output，默认 glb + ZUP_M + rad）；修改器栈经 evalModifiedMesh 求值后导出（非破坏性）。
function exportSceneFile(format) {
  const s = project.scene();
  if (!s) { status('无活动场景'); return; }
  const o = project.output || defaultOutputSettings();
  const axis = o.axis || (format === 'obj' ? 'YUP_M' : 'ZUP_M');
  let out;
  try {
    out = exportScene(s, {
      format, axis, name: project.name,
      rotationUnit: o.rotationUnit || 'rad',
      materialLibrary: project.materialLibrary || null,
      applyModifiersFn: (mesh, mods) => applyModifiers(mesh, mods),
    });
  } catch (e) { status('导出失败：' + e.message); return; }
  for (const f of out.files) {
    const blob = f.bytes ? new Blob([f.bytes], { type: 'application/octet-stream' })
      : new Blob([f.text], { type: f.name.endsWith('.mtl') ? 'text/plain' : (f.name.endsWith('.gltf') ? 'model/gltf+json' : 'text/plain') });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob); a.download = f.name; a.click();
    URL.revokeObjectURL(a.href);
  }
  status('已导出 ' + out.files.map(f => f.name).join(' + ') + '（' + out.axis + '，'
    + out.stats.objects + ' 对象 / ' + out.stats.vertices + ' 顶点 / ' + out.stats.triangles + ' 三角形）');
}
