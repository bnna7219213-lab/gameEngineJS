...
// M7/D29：把 World 数据块下发到 Viewport3D（ambient/fog/ibl/sunDir -> 灯光）。
// world 缺省时用 World.default()；向后兼容（旧场景无 world 字段则默认）。
export function applyWorldToViewport(vp, world) {
  const w = world || World.default();
  vp.setAmbient(w.ambient);
  vp.setFog(w.fog || null);
  if (w.ibl) vp.setIBL(w.ibl);
  // sunDir -> 方向光（若视口尚无方向光则补一个）
  if (w.sunDir && !vp.lights.some(L => L.type === 0)) {
    vp.setLights([...vp.lights, { type: 0, direction: w.sunDir, color: [1, 1, 1], intensity: 1 }]);
  }
}
...
export class ViewportPanel {
  ...
  syncScene(sceneOverride) {
    ...
    // D36：若命令总线存在且有脏标记，仅重建受影响对象；否则全量重建。
    const bus = this.ctx.commandBus;
    let dirty = null;
    if (bus && bus.dirtySet && bus.dirtySet.size > 0) dirty = bus.consumeDirty();
    if (dirty && dirty.ids.size > 0) return this._syncDirty(s, dirty);
  ...
  // 增量重建：仅重建 dirty.ids 中对象（其余复用现有视口 mesh）。
  _syncDirty(scene, dirty) {
    if (!scene) return;
    for (const objId of dirty.ids) {
      const o = scene.get(objId);
      if (!o) continue;
      // 移除旧视口 mesh（反向查 meshToObj）
      for (const [mid, oid] of [...this.meshToObj.entries()]) {
        if (oid === objId) { this.vp.removeMesh(mid); this.meshToObj.delete(mid); }
      }
      const mc = (o.components || {}).mesh;
      const tr = worldTransform(scene, o);
      if (!mc) { this._addMarker(o, tr); continue; }
      const cg = mc.customGeo;
      const geo = cg ? {
        positions: new Float32Array(cg.positions),
