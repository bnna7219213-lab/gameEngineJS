...
import { registry, componentsOf, addableComponents, getComponent, addComponent, removeComponent, setField, OUTPUT_FIELDS, defaultOutputSettings } from './registry.js';
...
export class InspectorPanel {
  ...
  render() {
    ...
    const obj = this.obj();
    if (!obj) { el.innerHTML = '<div class="placeholder">在层级或视口中选择一个对象</div>'; this._sceneCard(el); return; }
    const ctx = this.ctx;
    // 名称
  ...
  // 场景级「输出 Output」选项卡（v3 M-F / D33）：格式 + 坐标轴/单位 + 旋转单位，
  // 写入 project.output，由 main.js 的导出命令消费（导出场景 OBJ/glTF/GLB）。
  _sceneCard(el) {
    const ctx = this.ctx;
    const project = ctx.project;
    if (!project || !project.scene()) return;
    const o = project.output || (project.output = defaultOutputSettings());
    const card = document.createElement('div'); card.className = 'comp';
    const head = document.createElement('div'); head.className = 'comp-head';
    head.appendChild(document.createTextNode('输出 Output（场景级）'));
    ...
    for (const f of OUTPUT_FIELDS) {
      ...
      const sel = document.createElement('select');
      for (const op of f.options) { const opt = document.createElement('option'); opt.value = op; opt.textContent = op; sel.appendChild(opt); }
      sel.value = String(o[f.key] ?? f.options[0]);
      sel.onchange = () => { o[f.key] = sel.value; ctx.status('输出设置：' + f.label + ' = ' + sel.value); };
      row.appendChild(sel); card.appendChild(row);
    }
    const btn = document.createElement('button'); btn.className = 'add-row-btn';
    btn.textContent = '导出场景（' + o.format + '）';
    btn.onclick = () => { const c = ctx.commands && ctx.commands.find(x => x.id === 'export-' + o.format); if (c) c.run(); };
